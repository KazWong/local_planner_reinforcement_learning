...
class IsaacEnv(object):
	def __init__(self, world_name):
		...
		#start omniverse isaac sim
		startup_config = {
			"renderer": "RayTracedLighting",
			"headless": args.headless,
			"experience": f'{os.environ["EXP_PATH"]}/omni.isaac.sim.python.kit',
		}
		self.kit = OmniKitHelper(startup_config)
		...
		self.path = "/Library/Environments/"
		...
		#load environment
		env_path = nucleus + self.name + ".usd"
		print(env_path)
...
def Run(args):
	...
	while kit.app.is_running():
		...
		kit.update(1.0 / 60.0)
		#kit.update() # Run in realtime mode, we don't specify the step size
